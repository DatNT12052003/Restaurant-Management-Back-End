...
import { ICreateAccountPayload, IGetParams, IResponse } from "~/interfaces";
...
export const getAccounts = async (req: Request, res: Response) => {
    try {
        const currentPage = parseInt(req.query.page as string) || PAGINATION.DEFAULT_PAGE;
        ...
        const orderType = ((req.query.orderType as OrderTypeEnum) || "DESC").toUpperCase() === "ASC" ? "ASC" : "DESC";

        const response: IResponse<any> = await accountService.getAccounts({
            offset,
            limit,
            searchText,
            searchField,
